...
public sealed partial class MainPage : Page
{
    ...
    private async System.Threading.Tasks.Task TryRestoreLastFolderAsync()
    {
        ...
        if (!settingsService.RememberLastFolder || string.IsNullOrEmpty(settingsService.LastFolderPath))
        {
            // System.Diagnostics.Debug.WriteLine($"[MainPage] 路径为空或未启用，跳过恢�?);
            return;
        }
        ...
        {
            // System.Diagnostics.Debug.WriteLine($"[MainPage] 未找到上次路�? {settingsService.LastFolderPath}");
        }
    }
    ...
    private async System.Threading.Tasks.Task<FolderNode?> TryFindAndLoadNodeByPathAsync(string targetPath)
    {
        ...
        // System.Diagnostics.Debug.WriteLine($"[TryFindAndLoadNodeByPathAsync] 最终找到节�? {currentNode.Name}, 完整路径: {currentNode.FullPath}");
        return currentNode;
    }
        ...
        try
        {
            ...
            for (var i = 0; i < path.Count; i++)
            {
                ...
                if (i < path.Count - 1)
                {
                    if (!node.IsLoaded)
                    {
                        // System.Diagnostics.Debug.WriteLine($"[ExpandTreeViewPathAsync] 加载子节�? {node.Name}");
                        await ViewModel.LoadChildrenAsync(node);
                        if (_isUnloaded || AppLifetime.IsShuttingDown)
                        {
                            return;
    ...
    private async void DeleteButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (result == ContentDialogResult.Primary)
        {
            ...
            var deleteResult = await DeleteWorkflowHelper.DeleteFilesAsync(
                ...
                dialog,
                App.GetService<IThumbnailService>(),
                message => System.Diagnostics.Debug.WriteLine($"鍒犻櫎鏂囦欢澶辫�? {message}"));
